/**
 * Standard asset SVGs matching the provided email template images exactly.
 * These can be rendered directly or rasterized to high-resolution PNG Base64 Data URLs.
 */
...
export const EMAIL_ASSETS: Record<string, EmailAsset> = {
  'img/logo.png': {
    id: 'logo',
    name: '104 學習品牌 Logo',
    width: 136,
    height: 30,
    alt: '104 學習',
    svg: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 136 30" width="136" height="30">
  <defs>
  ...
  <path d="M 33 2 C 40 2 47 7.5 47 15 C 47 22.5 40 28 33 28 L 33 23 C 38 23 41.5 19.5 41.5 15 C 41.5 10.5 38 7 33 7 Z" fill="url(#orangeGrad)" clip-path="url(#rightHalfClip)" />
  <!-- '4' -->
  ...
  'img/header.png': {
    id: 'header',
    name: '頂部主視覺橫幅 (Header Banner)',
    width: 640,
    height: 220,
    alt: '9/17 HR 講座專屬通知：感謝參與講座，講座回放及 5 名免費簡報認證',
    svg: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 220" width="640" height="220">
  <defs>
  ...
  'img/gift.png': {
    ...
    height: 40,
    alt: '禮物',
    svg: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 36 40" width="36" height="40">
  <defs>
  ...
  'img/chart.png': {
    id: 'chart',
    name: '成長長條圖示 (Bar Chart)',
    width: 28,
    height: 30,
    alt: '成效圖示',
    svg: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 28 30" width="28" height="30">
  ...
  'img/step-people.png': {
    ...
    height: 22,
    alt: '人才導入',
    svg: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 22 22" width="22" height="22">
...
/**
 * Convert SVG string to a Data URL (image/svg+xml;utf8,...)
 */
...
/**
 * Cache for rasterized PNG data URLs
 */
const pngCache = new Map<string, string>();

/**
 * Converts an SVG string into a high-resolution PNG Base64 Data URL using HTML5 Canvas.
 * This ensures 100% compatibility with email clients like Gmail, Outlook, Apple Mail.
 */
export async function rasterizeSvgToPngDataUrl(
  ...
  const cacheKey = `${width}x${height}_s${scale}_${svgString.slice(0, 40)}`;
    ...
    try {
      ...
      img.onload = () => {
        ...
        if (ctx) {
          ...
          pngCache.set(cacheKey, pngDataUrl);
          resolve(pngDataUrl);
        } else {
          URL.revokeObjectURL(url);
